...
void calcDenseFlowGPU(string file_name, int bound, int type, int step, int dev_id,
	...
	if(flag != std::string::npos) {
		std::cout << "[INFO]Using video input!" << std::endl;
		...
		std::cout << "[INFO]Using frame input!" << std::endl;
        std::vector<string> frm_names;
        DIR *dp;
        struct dirent *dirp;
        if ((dp = opendir(file_name.c_str())) == NULL) {
            perror("[INFO]Opendir error!");
            exit(1);
        }
        while ((dirp = readdir(dp)) != NULL) {
            if((strcmp(".", dirp->d_name)==0) || (strcmp("..", dirp->d_name)==0))
                continue;
            frm_names.push_back(dirp->d_name);
        }
        closedir(dp);
        sort(frm_names.begin(), frm_names.end());
	    ...
	    while(true){
	        //build mats for the first frame
	        if (!initialized){
               capture_frame = cv::imread(file_name+"/"+frm_names[idx++]);
	           if (capture_frame.empty()) return; // read frames until end
